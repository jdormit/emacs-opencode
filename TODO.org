...
* TODO sometimes acp server http stuff is unresponsive?
* TODO follow up on agent messages streaming in above last user message
* TODO fix agent (plan/build) selection
* TODO fix (remove) acp timeout for prompts
* TODO fix reconnect logic now that we've switched to acp
* DONE fix session restore
CLOSED: [2026-03-24 Tue 00:00]
* DONE fix issue where you can't scroll around when a response is streaming in
CLOSED: [2026-03-24 Tue 00:09]
...
CLOSED: [2026-03-23 Sun]
Replaced the SSE-based architecture entirely with ACP (Agent Client Protocol)
over stdio.  Core operations (prompts, streaming, permissions, cancel) use
JSON-RPC over newline-delimited JSON via the =opencode acp= child process.
Supplementary operations (questions, session metadata, shell commands, auth)
use HTTP against the embedded server.  SSE module deleted entirely.
* TODO Display sub-agent tool calls in the session buffer
The ACP event stream only forwards events for sessions registered in its
session manager.  Sub-agent sessions are created server-side and never
registered, so their =message.part.updated= / =message.part.delta= events
are silently dropped.  Sub-agent permissions are handled via HTTP polling,
but tool call visibility requires a different approach (e.g., polling
sub-agent messages once we know the session ID from the task tool metadata).
