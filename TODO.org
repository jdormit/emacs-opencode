...
#+end_src
* TODO Handle question functionality
- question.asked SSE
...
#+end_src
* TODO Address poor performance of SSE buffer/event dispatch
* IN PROGRESS Buffer-aware commands
...
* IN PROGRESS Make the session buffer look pretty
With colors!

* TODO Handle session.diff messages
- Show each session.diff payload inside the message list.
- Provide a dedicated diff UI rendering (mirrors TUI behavior).

* TODO Support slash commands in the session buffer
...
* TODO Handle sub-agents/sub-sessions 
