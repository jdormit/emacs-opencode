...
* TODO Make open-session reuse the existing session buffer if there is one
* IN PROGRESS Buffer-aware commands
** DONE Ask about region/point/file context.
CLOSED: [2026-01-29 Thu 17:04]
** TODO Apply edits to selection or buffer.
** TODO send context to existing session buffers, not just new ones
...
* TODO Handle diff in edit tool calls
